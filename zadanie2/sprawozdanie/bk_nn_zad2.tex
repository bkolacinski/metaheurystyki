...
\begin{document}
    ...
    Program uruchamia się poprzez uruchomienie pliku main.py np. komendą \texttt{python main.py} w terminalu.
    Program umożliwia wybór jednej z czterech funkcji do optymalizacji: funkcji jednowymiarowej z sekcji 3 (przedział \([-150, 150]\)), funkcji dwuwymiarowej z sekcji 4 (obszar \([-3,12]\times[4.1,5.8]\)), lub własnej funkcji jedno- lub dwuwymiarowej.
    Przy definiowaniu własnej funkcji dostępne są standardowe funkcje matematyczne (sinus, cosinus, tangens, eksponens, logarytm, pierwiastek kwadratowy) oraz stałe (pi i e).
    Parametry algorytmu obejmują: liczbę epok, liczbę iteracji w epoce, temperaturę początkową, współczynnik alpha z przedziału \((0, 1)\) oraz stałą k.
    Po zakończeniu obliczeń program wyświetla znaleziony punkt (współrzędną x dla funkcji jednowymiarowej lub współrzędne x i y dla funkcji dwuwymiarowej), wartość funkcji w tym punkcie, numer iteracji oraz czas wykonania w milisekundach.
    

    \newpage
    \section{Odtworzenie eksperymentów z artykułu}\label{sec:eksperymenty}
    ...
    Znalezione przez nas maksimum funkcji \(f(x)\) to \(f(100.16) = 10.66\) (rysunek \ref{fig:wykres_sekcja_3}), co różni się od maksimum globalnego o \(\delta=|10.66-11|=0.34\).
    ...
    Zaimplementowany algorytm symulowanego wyżarzania jest skuteczną metodą znajdowania rozwiązań bliskich optymalnym, co potwierdzają przeprowadzone eksperymenty.
    Nie gwarantuje on jednak znalezienia globalnego maksimum funkcji.

    Otrzymane przez nas wyniki różnią się od wyników przedstawionych w artykule, co może wynikać z różnych implementacji algorytmu lub losowego charakteru algorytmu.
    Wyniki otrzymane przez nas (widoczne w sekcji \ref{sec:eksperymenty}) były celowo dobierane tak, żeby znalezione przez nas rozwiązanie nie pokrywało się 100\% z
    maksimum globalnym (aby zwiększyć czytelność wykresów), stąd możliwe są różnice w wynikach (wyniki w artykule były bardzo dokładne).

    Kluczowe dla efektywności algorytmu jest odpowiednie dobranie parametrów:
    \begin{itemize}
        \item \textbf{Temperatura początkowa} ma istotny wpływ na jakość rozwiązania, nie wpływając na czas obliczeń.
        Wyższa wartość pozwala na szerszą eksplorację przestrzeni rozwiązań.
        Zbyt mała wartość może prowadzić do utknięcia w lokalnym ekstremum.
        \item \textbf{Współczynnik chłodzenia (\(\alpha\))} decyduje o tempie obniżania temperatury, co wpływa zarówno na czas działania algorytmu, jak i na dokładność znalezionego rozwiązania.
        Jego wartość pozwala znaleźć równowagę między szybkością zbieżności a ryzykiem przedwczesnego utknięcia w lokalnym ekstremum.
        \item \textbf{Liczba epok} wprost przekłada się na czas wykonania i liczbę iteracji.
        Zwiększenie tej wartości zazwyczaj poprawia jakość wyniku, lecz wiąże się z większym kosztem obliczeniowym.
    \end{itemize}
