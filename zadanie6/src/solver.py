...
def _build_3opt_route(
    ...
    """Build a new route after a 3-opt move."""
    s1 = route[:i].tolist()
    s2 = route[i:j].tolist()
    s3 = route[j:k].tolist()
    s4 = route[k:].tolist()
...
def _check_route_feasibility(
    ...
    """Check if a route satisfies all time windows."""
...
def find_best_3opt_move_tw(
    ...
    """
    Find the best 3-opt move considering both distance and time windows.

    Returns [delta_distance, i, j, k, move_type, is_feasible]
    """
    n = len(route)
            ...
            for k in range(j + 1, n):
                ...
                for move_type in range(1, 8):
                    ...
                    feasible = _check_route_feasibility(
                        new_route, distance_matrix, service_times, window_starts, window_ends, speed_factor
                    )
...
def find_best_3opt_move(
    ...
    """Original distance-only 3-opt move finder (kept for compatibility)."""
    n = len(route)
...
def three_opt(
    ...
    """Original 3-opt without time window consideration."""
...
def three_opt_tw(
    ...
    """
    3-opt with time window feasibility checking.

    Only accepts moves that maintain time window feasibility.
    Prioritizes feasible moves over infeasible ones.
    """
    route = np.array(initial_route, dtype=np.int64)
    while True:
        best_move = find_best_3opt_move_tw(
            route, dist_matrix, service_times, window_starts, window_ends, speed_factor
        )
        # Accept if move improves distance and is feasible
...
if __name__ == "__main__":
    ...
    print(
        f"Liczba klientow: {len(data['coords']) - 1}"
    )  # -1 bo depot to indeks 0

    # Oblicz macierz odlegosci
    dist_mtx = calculate_distance_matrix(data["coords"])

    # Utworz poczatkowa trase (wszystkie punkty w kolejnosci)
    start_route = list(range(len(data["coords"])))
    np.random.shuffle(
        start_route[1:]
    )  # Tasuj wszystko oprocz depotu (indeks 0)
